...
import { listPublicShows } from '@/lib/api';
...
const HomePage = () => {
  ...
  const venues = useMemo(() => {
    ...
    for (const s of shows) {
      if (s.venues && !map.has(s.venues.id)) map.set(s.venues.id, s.venues);
    }
    return Array.from(map.values());
  }, [shows]);

  const filteredShows = selectedVenueId ? shows.filter((s) => s.venues?.id === selectedVenueId) : shows;
        ...
        <div className="container py-12 flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredShows.map((show, i) => {
              const cover = show.event_images?.[0]?.url;
              const funcionCount = show.funciones.length;
              return (
                <motion.div
                  key={show.id}
                    ...
                    <Card className="ticket-stub hover:border-gold/60 transition-colors h-full overflow-hidden relative">
                      {(show.isPast || show.isSoldOut) && (
                        <span className="absolute top-3 right-3 z-10 bg-destructive text-destructive-foreground text-xs font-bold px-3 py-1 rounded-full rotate-3 shadow-lg">
                          {show.isPast ? 'EVENTO PASADO' : 'AGOTADO'}
                        </span>
                      )}
                      {cover ? (
                        <div className="aspect-video w-full overflow-hidden">
                          <img src={cover} alt={show.title} className="w-full h-full object-cover" />
                        </div>
                      ) : (
                      ...
                      <CardContent className="p-6 flex flex-col gap-3">
                        <h3 className="font-display text-xl leading-snug">{show.title}</h3>
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
                          <CalendarDays className="h-4 w-4" />
                          {show.nextFuncion ? formatDateTime(show.nextFuncion.event_date) : '—'}
                          {funcionCount > 1 && (
                            <span className="text-xs text-gold">+{funcionCount - 1} función{funcionCount - 1 > 1 ? 'es' : ''} más</span>
                          )}
                        </div>
                        <div className="flex items-center gap-2 text-sm text-muted-foreground">
